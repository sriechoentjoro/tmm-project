 ...
 * @var \App\Model\Entity\App\Model\Entity\ApprenticeRecordCoeVisa|null $apprenticeRecordCoeVisa */
...
<nav class="actions-sidebar" id="actions-sidebar">
    ...
    <ul class="side-nav">
        ...
        <li><?= $this->Html->link('<i class="fas fa-list"></i> ' . __('List Apprentice Record Coe Visas'), ['action' => 'index'], ['escape' => false]) ?></li>
...
<!-- Main Content -->
<div class="apprenticeRecordCoeVisas form content">
    <div class="card">
        <div class="content-header">
            <h3 class="content-title">
                <i class="fas fa-edit"></i>
                <?= __(Inflector::humanize($this->request->getParam("action")) . ' Apprentice Record Coe Visa') ?>
            </h3>
        </div>

        <div class="card-body">
            <?= $this->Form->create($apprenticeRecordCoeVisa, ['data-confirm' => 'true', 'id' => 'apprenticeRecordCoeVisaForm']) ?>
            <?php if (!empty($apprenticeRecordCoeVisa->id)): ?>
                <?= $this->Form->hidden('id') ?>
            <?php endif; ?>
            <fieldset>
                <legend><?= __('Enter Apprentice Record Coe Visa Information') ?></legend>
                <div class="row">
                    ...
                    <div class="col-12 mb-3">
                        <label class="form-label"><?= __('Master Coe Type Id') ?></label>
                        <?= $this->Form->control('master_coe_type_id', [
                            'class' => 'form-control',
                            'placeholder' => __('Enter Master Coe Type Id'),
                            'label' => false
                        ]) ?>
                    </div>
                    <div class="col-12 mb-3">
                        <label class="form-label"><?= __('Date Coe Received') ?></label>
                        <?= $this->Form->control('date_coe_received', [
                            'type' => 'text',
                            'class' => 'form-control datepicker',
                            'placeholder' => __('Select Date Coe Received'),
                            'label' => false,
                            'autocomplete' => 'off'
                        ]) ?>
                    </div>
                    <div class="col-12 mb-3">
                        <label class="form-label"><?= __('Date Visa Application') ?></label>
                        <?= $this->Form->control('date_visa_application', [
                            'type' => 'text',
                            'class' => 'form-control datepicker',
                            'placeholder' => __('Select Date Visa Application'),
                            'label' => false,
                            'autocomplete' => 'off'
                        ]) ?>
                    </div>
                    <div class="col-12 mb-3">
                        <label class="form-label"><?= __('Date Visa Received') ?></label>
                        <?= $this->Form->control('date_visa_received', [
                            'type' => 'text',
                            'class' => 'form-control datepicker',
                            'placeholder' => __('Select Date Visa Received'),
                            'label' => false,
                            'autocomplete' => 'off'
                        ]) ?>
                    </div>
                    <div class="col-12 mb-3">
                        <label class="form-label"><?= __('Place Visa Issued') ?></label>
                        <?= $this->Form->control('place_visa_issued', [
                            'class' => 'form-control',
                            'placeholder' => __('Enter Place Visa Issued'),
                            'label' => false
                        ]) ?>
                    </div>
                </div>
            ...
            <div class="form-actions mt-4">
                <?= $this->Form->button(__('Save Apprentice Record Coe Visa'), [
