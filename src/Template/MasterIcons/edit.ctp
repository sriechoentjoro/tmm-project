 ...
 * @var \App\Model\Entity\App\Model\Entity\MasterIcon|null $masterIcon */
...
<nav class="actions-sidebar" id="actions-sidebar">
    ...
    <ul class="side-nav">
        ...
        <li><?= $this->Html->link('<i class="fas fa-list"></i> ' . __('List Master Icons'), ['action' => 'index'], ['escape' => false]) ?></li>
    </ul>
</nav>

<!-- Main Content -->
<div class="masterIcons form content">
    <div class="card">
        <div class="content-header">
            <h3 class="content-title">
                <i class="fas fa-edit"></i>
                <?= __(Inflector::humanize($this->request->getParam("action")) . ' Master Icon') ?>
            </h3>
        </div>

        <div class="card-body">
            <?= $this->Form->create($masterIcon, ['data-confirm' => 'true', 'id' => 'masterIconForm']) ?>
            <?php if (!empty($masterIcon->id)): ?>
                <?= $this->Form->hidden('id') ?>
            <?php endif; ?>
            <fieldset>
                <legend><?= __('Enter Master Icon Information') ?></legend>
                <div class="row">
            ...
            <div class="form-actions mt-4">
                <?= $this->Form->button(__('Save Master Icon'), [
