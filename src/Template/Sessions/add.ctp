 ...
 * @var \App\Model\Entity\App\Model\Entity\Session|null $session */
...
<nav class="actions-sidebar" id="actions-sidebar">
    ...
    <ul class="side-nav">
        ...
        <li><?= $this->Html->link('<i class="fas fa-list"></i> ' . __('List Sessions'), ['action' => 'index'], ['escape' => false]) ?></li>
    </ul>
</nav>
    ...
    <div class="card">
        <div class="content-header">
            <h3 class="content-title">
                ...
                <?= __(Inflector::humanize($this->request->getParam("action")) . ' Session') ?>
            </h3>
        </div>

        <div class="card-body">
            <?= $this->Form->create($session, ['data-confirm' => 'true', 'id' => 'sessionForm']) ?>
            <fieldset>
                <legend><?= __('Enter Session Information') ?></legend>
                <div class="row">
                    <div class="col-12 mb-3">
                        <label class="form-label"><?= __('Expires') ?></label>
                        <?= $this->Form->control('expires', [
                            'class' => 'form-control',
                            'placeholder' => __('Enter Expires'),
                            'label' => false
                        ]) ?>
                    </div>
                </div>
