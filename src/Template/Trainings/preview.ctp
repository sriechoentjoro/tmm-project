 ...
 * @var \App\Model\Entity\App\Model\Entity\Training $training * @var array $validationErrors
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <?php foreach ($fieldMetadata as $field => $meta): ?>
                ...
                if ($isForeignKey && !empty($training->id)) {
                    ...
                    // Try singular first
                    if ($training->has($associationProperty)) {
                        $assoc = $training->get($associationProperty);
                        if (is_object($assoc)) {
                    ...
                    if (!$assocFound && $training->has($associationPropertyPlural)) {
                        $assoc = $training->get($associationPropertyPlural);
                        if (is_object($assoc)) {
                            if (isset($assoc->title)) {
                                $displayValue = h($assoc->title);
                                $assocFound = true;
            ...
            <div class="form-actions">
                <?php if (empty($validationErrors)): ?>
                    <?= $this->Form->create($training, ['url' => ['action' => 'edit', $training->id]]) ?>
                    <?= $this->Form->hidden('id') ?>
                ...
                <?= $this->Html->link(__('Back to Edit'), 
                    ['action' => 'edit', isset($training->id) ? $training->id : null], 
