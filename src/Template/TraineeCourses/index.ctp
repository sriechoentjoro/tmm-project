 ...
 * @var \App\Model\Entity\TraineeCourse[]|\Cake\Collection\CollectionInterface $traineeCourses
 */
?>
    ...
    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 15px;">
        <div style="display: flex; align-items: center; gap: 10px;">
            ...
            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                <?= $this->Html->link(
                    '<i class="fas fa-list"></i> ' . __('List Trainee Courses'),
                    ['action' => 'index'],
                    ['class' => 'dropdown-item', 'escape' => false]
                ) ?>
                <?= $this->Html->link(
                    '<i class="fas fa-plus"></i> ' . __('New Trainee Course'),
                    ['action' => 'add'],
                    ['class' => 'dropdown-item', 'escape' => false]
                ) ?>
            ...
            <h2 style="margin: 0; display: inline-block;"><?= __('Trainee Courses') ?></h2>
        </div>
...
<div class="table-scroll-wrapper" style="overflow-x: auto; cursor: grab; -webkit-overflow-scrolling: touch; user-select: none;">
    <div class="traineeCourses index content">
        <table class="table" style="border-collapse: collapse; width: 100%; min-width: 800px;">
            ...
            <thead style="background: linear-gradient(135deg, rgba(102, 126, 234, 0.15) 0%, rgba(118, 75, 162, 0.15) 100%); position: sticky; top: 0; z-index: 10;">
                <tr>
                    ...
                    <th style="padding: 12px; border-bottom: 2px solid #667eea; white-space: nowrap;" scope="col"><?= $this->Paginator->sort('course_major_id') ?></th>
                    <th style="padding: 12px; border-bottom: 2px solid #667eea; white-space: nowrap;" scope="col"><?= $this->Paginator->sort('course_year') ?></th>
            ...
            <tr class="filter-row" style="background-color: #f8f9fa;">
                ...
                <td style="padding: 5px;">
                    <select class="filter-input form-control form-control-sm" data-column="course_major_id" data-type="select" style="font-size: 0.85rem; padding: 4px;">
                        <option value="">All Course_majors</option>
                        <?php foreach ($course_majors as $id => $name): ?>
                            <option value="<?= $id ?>"><?= h($name) ?></option>
                        <?php endforeach; ?>
                    </select>
                </td>
                <td style="padding: 5px;">
                    <select class="filter-operator form-control form-control-sm" data-column="course_year" style="font-size: 0.75rem; padding: 2px 4px; margin-bottom: 2px;">
                        <option value="=">=</option>
                        <option value="!=">!=</option>
                        <option value="<">&lt;</option>
                        <option value=">">&gt;</option>
                    ...
                    <input type="number" class="filter-input form-control form-control-sm" placeholder="Filter..." data-column="course_year" style="font-size: 0.85rem; padding: 4px;">
                    <input type="number" class="filter-input-range form-control form-control-sm" placeholder="To..." data-column="course_year" style="display:none; margin-top: 2px; font-size: 0.85rem;">
                </td>
                <td style="padding: 5px;">
            ...
            <tbody>
                <?php foreach ($traineeCourses as $traineeCourse): ?>
                <tr>
                    ...
                    <td class="actions" style="white-space: nowrap; padding: 8px;">
                        <div class="action-buttons-hover">
                            <?= $this->Html->link(
                                ...
                                ['action' => 'edit', $traineeCourse->id],
                                ['class' => 'btn-action-icon btn-edit-icon', 'escape' => false, 'title' => 'Edit']
                            ) ?>
                            <?= $this->Html->link(
                                '<i class="fas fa-eye"></i>',
                                ['action' => 'view', $traineeCourse->id],
                                ['class' => 'btn-action-icon btn-view-icon', 'escape' => false, 'title' => 'View']
                            ) ?>
                        </div>
                    </td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $this->Number->format($traineeCourse->id) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $traineeCourse->has('trainee') ? $this->Html->link($traineeCourse->trainee->name, ['controller' => 'Trainees', 'action' => 'view', $traineeCourse->trainee->id]) : '' ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $traineeCourse->has('vocational_training_institution') ? $this->Html->link($traineeCourse->vocational_training_institution->name, ['controller' => 'VocationalTrainingInstitutions', 'action' => 'view', $traineeCourse->vocational_training_institution->id]) : '' ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $this->Number->format($traineeCourse->course_major_id) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $this->Number->format($traineeCourse->course_year) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($traineeCourse->title) ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
