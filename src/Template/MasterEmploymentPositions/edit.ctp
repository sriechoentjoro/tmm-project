 ...
 * @var \App\Model\Entity\App\Model\Entity\MasterEmploymentPosition|null $masterEmploymentPosition */
...
<nav class="actions-sidebar" id="actions-sidebar">
    ...
    <ul class="side-nav">
        ...
        <li><?= $this->Html->link('<i class="fas fa-list"></i> ' . __('List Master Employment Positions'), ['action' => 'index'], ['escape' => false]) ?></li>
    </ul>
</nav>

<!-- Main Content -->
<div class="masterEmploymentPositions form content">
    <div class="card">
        <div class="content-header">
            <h3 class="content-title">
                <i class="fas fa-edit"></i>
                <?= __(Inflector::humanize($this->request->getParam("action")) . ' Master Employment Position') ?>
            </h3>
        </div>

        <div class="card-body">
            <?= $this->Form->create($masterEmploymentPosition, ['data-confirm' => 'true', 'id' => 'masterEmploymentPositionForm']) ?>
            <?php if (!empty($masterEmploymentPosition->id)): ?>
                <?= $this->Form->hidden('id') ?>
            <?php endif; ?>
            <fieldset>
                <legend><?= __('Enter Master Employment Position Information') ?></legend>
                <div class="row">
                    ...
                    <div class="col-12 mb-3">
                        <label class="form-label"><?= __('Cost Per Hour') ?></label>
                        <?= $this->Form->control('cost_per_hour', [
                            'class' => 'form-control',
                            'placeholder' => __('Enter Cost Per Hour'),
                            'label' => false
                        ]) ?>
                    </div>
                    <div class="col-12 mb-3">
            ...
            <div class="form-actions mt-4">
                <?= $this->Form->button(__('Save Master Employment Position'), [
