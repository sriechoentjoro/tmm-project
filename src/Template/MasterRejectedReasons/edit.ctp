 ...
 * @var \App\Model\Entity\App\Model\Entity\MasterRejectedReason|null $masterRejectedReason */
...
<nav class="actions-sidebar" id="actions-sidebar">
    ...
    <ul class="side-nav">
        ...
        <li><?= $this->Html->link('<i class="fas fa-list"></i> ' . __('List Master Rejected Reasons'), ['action' => 'index'], ['escape' => false]) ?></li>
    </ul>
</nav>

<!-- Main Content -->
<div class="masterRejectedReasons form content">
    <div class="card">
        <div class="content-header">
            <h3 class="content-title">
                <i class="fas fa-edit"></i>
                <?= __(Inflector::humanize($this->request->getParam("action")) . ' Master Rejected Reason') ?>
            </h3>
        </div>

        <div class="card-body">
            <?= $this->Form->create($masterRejectedReason, ['data-confirm' => 'true', 'id' => 'masterRejectedReasonForm']) ?>
            <?php if (!empty($masterRejectedReason->id)): ?>
                <?= $this->Form->hidden('id') ?>
            <?php endif; ?>
            <fieldset>
                <legend><?= __('Enter Master Rejected Reason Information') ?></legend>
                <div class="row">
                    ...
                    <div class="col-12 mb-3">
                        <label class="form-label"><?= __('Title Jpg') ?></label>
                        <?= $this->Form->control('title_jpg', [
                            'class' => 'form-control',
                            'placeholder' => __('Enter Title Jpg'),
                            'label' => false
                        ]) ?>
                    </div>
                    <div class="col-12 mb-3">
            ...
            <div class="form-actions mt-4">
                <?= $this->Form->button(__('Save Master Rejected Reason'), [
