 ...
 * @var \App\Model\Entity\App\Model\Entity\ApprenticeSubmissionDocument|null $apprenticeSubmissionDocument */
...
<nav class="actions-sidebar" id="actions-sidebar">
    ...
    <ul class="side-nav">
        ...
        <li><?= $this->Html->link('<i class="fas fa-list"></i> ' . __('List Apprentice Submission Documents'), ['action' => 'index'], ['escape' => false]) ?></li>
        ...
        <li><?= $this->Html->link('<i class="fas fa-list"></i> ' . __('List MasterDocumentSubmissionStatuses'), ['controller' => 'MasterDocumentSubmissionStatuses', 'action' => 'index'], ['escape' => false]) ?></li>
        <li><?= $this->Html->link('<i class="fas fa-plus"></i> ' . __('New MasterDocumentSubmissionStatus'), ['controller' => 'MasterDocumentSubmissionStatuses', 'action' => 'add'], ['escape' => false]) ?></li>
    </ul>
</nav>

<!-- Main Content -->
<div class="apprenticeSubmissionDocuments form content">
    <div class="card">
        <div class="content-header">
            <h3 class="content-title">
                <i class="fas fa-edit"></i>
                <?= __(Inflector::humanize($this->request->getParam("action")) . ' Apprentice Submission Document') ?>
            </h3>
        </div>

        <div class="card-body">
            <?= $this->Form->create($apprenticeSubmissionDocument, ['type' => 'file', 'data-confirm' => 'true', 'id' => 'apprenticeSubmissionDocumentForm']) ?>
            <?php if (!empty($apprenticeSubmissionDocument->id)): ?>
                <?= $this->Form->hidden('id') ?>
            <?php endif; ?>
            <fieldset>
                <legend><?= __('Enter Apprentice Submission Document Information') ?></legend>
                <div class="row">
                    ...
                    <div class="col-12 mb-3">
                        <label class="form-label"><?= __('Apprenticeship Submission Document Id') ?></label>
                        <?= $this->Form->control('apprenticeship_submission_document_id', [
                            'class' => 'form-control',
                            'placeholder' => __('Enter Apprenticeship Submission Document Id'),
                            'label' => false
                        ]) ?>
                    </div>
                    <div class="col-12 mb-3">
                        ...
                        <?php if (!empty($apprenticeSubmissionDocument->file_path)): ?>
                            <div class="mt-2">
                                <a href="<?= $this->Url->build('/' . $apprenticeSubmissionDocument->file_path, ['fullBase' => true]) ?>" 
                                   target="_blank" 
                    ...
                    <div class="col-12 mb-3">
                        <label class="form-label"><?= __('Master Document Submission Status Id') ?></label>
                        <?= $this->Form->control('master_document_submission_status_id', [
                            'options' => $masterDocumentSubmissionStatuses,
                            'class' => 'form-control',
                            'label' => false,
                            'empty' => __('-- Select Master Document Submission Status Id --')
                        ]) ?>
                    </div>
                    <div class="col-12 mb-3">
                        <label class="form-label"><?= __('Uploaded By') ?></label>
                        <?= $this->Form->control('uploaded_by', [
                            'class' => 'form-control',
                            'placeholder' => __('Enter Uploaded By'),
                            'label' => false
                        ]) ?>
                    </div>
                    <div class="col-12 mb-3">
                        <label class="form-label"><?= __('Uploaded At') ?></label>
                        <?= $this->Form->control('uploaded_at', [
                            'class' => 'form-control',
                            'placeholder' => __('Enter Uploaded At'),
                            'label' => false
                        ]) ?>
                    </div>
                    <div class="col-12 mb-3">
            ...
            <div class="form-actions mt-4">
                <?= $this->Form->button(__('Save Apprentice Submission Document'), [
