...
$(document).ready(function() {
    ...
    $('#<%= $singularVar %>Form').on('keydown', function(e) {
    ...
    $('#<%= $singularVar %>Form').on('submit', function(e) {
        ...
        setTimeout(function() {
            if (isSubmitting) {
                ...
                $('#<%= $singularVar %>Form').find('input, select, textarea, button').prop('disabled', false);
                $('.form-overlay').remove();
        }, 10000);
    });
    ...
    $(window).on('pageshow', function(event) {
        if (event.originalEvent.persisted) {
            isSubmitting = false;
            $('#submitBtn').prop('disabled', false).text('<?= __('Save <%= $singularHumanName %>') ?>').css('opacity', '1');
            $('#<%= $singularVar %>Form').find('input, select, textarea, button').prop('disabled', false);
            $('.form-overlay').remove();
    });
});
