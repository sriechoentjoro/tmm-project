 ...
 * @var \App\Model\Entity\App\Model\Entity\MasterPropinsi|null $masterPropinsi */
...
<nav class="actions-sidebar" id="actions-sidebar">
    ...
    <ul class="side-nav">
        ...
        <li><?= $this->Html->link('<i class="fas fa-list"></i> ' . __('List Master Propinsis'), ['action' => 'index'], ['escape' => false]) ?></li>
    </ul>
</nav>

<!-- Main Content -->
<div class="masterPropinsis form content">
    <div class="card">
        <div class="content-header">
            <h3 class="content-title">
                <i class="fas fa-edit"></i>
                <?= __(Inflector::humanize($this->request->getParam("action")) . ' Master Propinsi') ?>
            </h3>
        </div>

        <div class="card-body">
            <?= $this->Form->create($masterPropinsi, ['data-confirm' => 'true', 'id' => 'masterPropinsiForm']) ?>
            <?php if (!empty($masterPropinsi->id)): ?>
                <?= $this->Form->hidden('id') ?>
            <?php endif; ?>
            <fieldset>
            ...
            <div class="form-actions mt-4">
                <?= $this->Form->button(__('Save Master Propinsi'), [
