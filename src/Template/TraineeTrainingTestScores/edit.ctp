...
.score-section-title {
    ...
    color: #00796B; margin-bottom: 14px;
...
.score-bar-bg { height:8px; border-radius:4px; background:#e2e8f0; overflow:hidden; }
.score-bar-fill {
...
.grade-legend { display:flex; gap:6px; flex-wrap:wrap; margin-top:10px; }
.grade-chip {
    font-size:11px; font-weight:600; padding:2px 10px; border-radius:12px;
    border:1px solid currentColor; opacity:0.55; transition:opacity 0.2s;
}
...
.btn-save:hover { transform:translateY(-1px); box-shadow:0 4px 14px rgba(0,137,123,0.35); color:#fff; text-decoration:none; }
.btn-cancel {
    display:inline-flex; align-items:center; gap:7px;
...
<nav class="actions-sidebar" id="actions-sidebar">
    ...
    <ul class="side-nav">
        ...
        <li><?= $this->Html->link('<i class="fas fa-check-double"></i> ' . __('Pass/Fail Report'), ['action' => 'report'], ['escape' => false]) ?></li>
    </ul>
</nav>
...
<div class="traineeTrainingTestScores form content">
    ...
    <div class="score-card">
        ...
        <div class="score-header">
            <div class="score-header-icon"><i class="fas fa-pen"></i></div>
            <div>
                <h3><?= __('Edit Test Score') ?></h3>
                <p><?= __('Update the competency test result for this trainee') ?></p>
            </div>
            <span class="rec-id"># <?= h($traineeTrainingTestScore->id) ?></span>
        </div>
        ...
        <div class="form-actions">
            <?= $this->Form->button('<i class="fas fa-save"></i> ' . __('Save Changes'), [
                'type'   => 'submit',
                'class'  => 'btn-save',
                'id'     => 'submitBtn',
                'escape' => false,
...
$(document).ready(function () {
    ...
    function updateGrade() {
        ...
        $('#gradeBadge')
            .removeClass('g-A g-B g-C g-D g-E').addClass(g.cls)
