 ...
 * @var \App\Model\Entity\App\Model\Entity\MasterTranslation|null $masterTranslation */
...
<nav class="actions-sidebar" id="actions-sidebar">
    ...
    <ul class="side-nav">
        ...
        <li><?= $this->Html->link('<i class="fas fa-list"></i> ' . __('List Master Translations'), ['action' => 'index'], ['escape' => false]) ?></li>
    </ul>
</nav>

<!-- Main Content -->
<div class="masterTranslations form content">
    <div class="card">
        <div class="content-header">
            <h3 class="content-title">
                <i class="fas fa-edit"></i>
                <?= __(Inflector::humanize($this->request->getParam("action")) . ' Master Translation') ?>
            </h3>
        </div>

        <div class="card-body">
            <?= $this->Form->create($masterTranslation, ['data-confirm' => 'true', 'id' => 'masterTranslationForm']) ?>
            <?php if (!empty($masterTranslation->id)): ?>
                <?= $this->Form->hidden('id') ?>
            <?php endif; ?>
            <fieldset>
                <legend><?= __('Enter Master Translation Information') ?></legend>
                <div class="row">
                    ...
                    <div class="col-12 mb-3">
                        <label class="form-label"><?= __('Jpn') ?></label>
                        <?= $this->Form->control('jpn', [
                            'class' => 'form-control',
                            'placeholder' => __('Enter Jpn'),
                            'label' => false
                        ]) ?>
                    </div>
                    <div class="col-12 mb-3">
                        <label class="form-label"><?= __('Ind') ?></label>
                        <?= $this->Form->control('ind', [
                            'class' => 'form-control',
                            'placeholder' => __('Enter Ind'),
                            'label' => false
                        ]) ?>
                    </div>
                    <div class="col-12 mb-3">
                        <label class="form-label"><?= __('Eng') ?></label>
                        <?= $this->Form->control('eng', [
                            'class' => 'form-control',
                            'placeholder' => __('Enter Eng'),
                            'label' => false
                        ]) ?>
                    </div>
                </div>
            ...
            <div class="form-actions mt-4">
                <?= $this->Form->button(__('Save Master Translation'), [
