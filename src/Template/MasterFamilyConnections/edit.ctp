 ...
 * @var \App\Model\Entity\App\Model\Entity\MasterFamilyConnection|null $masterFamilyConnection */
...
<nav class="actions-sidebar" id="actions-sidebar">
    ...
    <ul class="side-nav">
        ...
        <li><?= $this->Html->link('<i class="fas fa-list"></i> ' . __('List Master Family Connections'), ['action' => 'index'], ['escape' => false]) ?></li>
    </ul>
</nav>

<!-- Main Content -->
<div class="masterFamilyConnections form content">
    <div class="card">
        <div class="content-header">
            <h3 class="content-title">
                <i class="fas fa-edit"></i>
                <?= __(Inflector::humanize($this->request->getParam("action")) . ' Master Family Connection') ?>
            </h3>
        </div>

        <div class="card-body">
            <?= $this->Form->create($masterFamilyConnection, ['data-confirm' => 'true', 'id' => 'masterFamilyConnectionForm']) ?>
            <?php if (!empty($masterFamilyConnection->id)): ?>
                <?= $this->Form->hidden('id') ?>
            <?php endif; ?>
            <fieldset>
                <legend><?= __('Enter Master Family Connection Information') ?></legend>
                <div class="row">
                    <div class="col-12 mb-3">
                        <label class="form-label"><?= __('Family Category') ?></label>
                        <?= $this->Form->control('family_category', [
                            'class' => 'form-control',
                            'placeholder' => __('Enter Family Category'),
                            'label' => false
                        ]) ?>
                    </div>
                    <div class="col-12 mb-3">
                        <label class="form-label"><?= __('Title Eng') ?></label>
                        <?= $this->Form->control('title_eng', [
                            'class' => 'form-control',
                            'placeholder' => __('Enter Title Eng'),
                            'label' => false
                        ]) ?>
                    </div>
                    <div class="col-12 mb-3">
                    ...
                    <div class="col-12 mb-3">
                        <label class="form-label"><?= __('Title Kanji') ?></label>
                        <?= $this->Form->control('title_kanji', [
                            'class' => 'form-control',
                            'placeholder' => __('Enter Title Kanji'),
                            'label' => false
                        ]) ?>
                    </div>
                    <div class="col-12 mb-3">
                        <label class="form-label"><?= __('Updated') ?></label>
                        <?= $this->Form->control('updated', [
                            'type' => 'text',
                            'class' => 'form-control datepicker',
                            'placeholder' => __('Select Updated'),
                            'label' => false,
                            'autocomplete' => 'off'
                        ]) ?>
                    </div>
            ...
            <div class="form-actions mt-4">
                <?= $this->Form->button(__('Save Master Family Connection'), [
