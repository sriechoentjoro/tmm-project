 ...
 * @var \App\Model\Entity\App\Model\Entity\MasterInterviewResult $masterInterviewResult * @var array $validationErrors
...
<div class="masterInterviewResults preview content">
    <div class="card">
        ...
        <div class="card-body">
            ...
            <?php foreach ($fieldMetadata as $field => $meta): ?>
                ...
                if ($isForeignKey && !empty($masterInterviewResult->id)) {
                    ...
                    // Try singular first
                    if ($masterInterviewResult->has($associationProperty)) {
                        $assoc = $masterInterviewResult->get($associationProperty);
                        if (is_object($assoc)) {
                    ...
                    if (!$assocFound && $masterInterviewResult->has($associationPropertyPlural)) {
                        $assoc = $masterInterviewResult->get($associationPropertyPlural);
                        if (is_object($assoc)) {
                            if (isset($assoc->title)) {
                                $displayValue = h($assoc->title);
                                $assocFound = true;
            ...
            <div class="form-actions">
                <?php if (empty($validationErrors)): ?>
                    <?= $this->Form->create($masterInterviewResult, ['url' => ['action' => 'edit', $masterInterviewResult->id]]) ?>
                    <?= $this->Form->hidden('id') ?>
                ...
                <?= $this->Html->link(__('Back to Edit'), 
                    ['action' => 'edit', isset($masterInterviewResult->id) ? $masterInterviewResult->id : null], 
