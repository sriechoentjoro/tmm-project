...
<script src="http://localhost/static-assets/js/form-confirm.js?v=2.0"></script>
...
$(document).ready(function() {
    ...
    // Nested Address Select (Province -> Kabupaten -> Kecamatan -> Kelurahan)
    // Province change - load Kabupaten
    $('#<%= Inflector::classify($singularVar) %>PropinsiId').change(function() {
        var propinsiId = $(this).val();
        var kabupatenSelect = $('#<%= Inflector::classify($singularVar) %>KabupatenId');
        var kecamatanSelect = $('#<%= Inflector::classify($singularVar) %>KecamatanId');
        var kelurahanSelect = $('#<%= Inflector::classify($singularVar) %>KelurahanId');
        ...
        if (propinsiId) {
            ...
            $.ajax({
                type: 'POST',
                url: '<?= $this->Url->build(["controller" => "Daerahs", "action" => "optionKabupaten"]) ?>/id_propinsi:' + propinsiId,
                cache: false,
                success: function(html) {
                    kabupatenSelect.html(html);
                    $('.address-loading').hide();
                },
                error: function() {
                    $('.address-loading').hide();
                    alert('Error loading Kabupaten data');
            });
    });
    
    // Kabupaten change - load Kecamatan
    $('#<%= Inflector::classify($singularVar) %>KabupatenId').change(function() {
        var propinsiId = $('#<%= Inflector::classify($singularVar) %>PropinsiId').val();
        ...
        if (propinsiId && kabupatenId) {
            ...
            $.ajax({
                type: 'POST',
                url: '<?= $this->Url->build(["controller" => "Daerahs", "action" => "optionKecamatan"]) ?>/id_propinsi:' + propinsiId + '/id_kabupaten:' + kabupatenId,
                cache: false,
                success: function(html) {
                    kecamatanSelect.html(html);
                    $('.address-loading').hide();
                },
                error: function() {
                    $('.address-loading').hide();
                    alert('Error loading Kecamatan data');
            });
    });
    
    // Kecamatan change - load Kelurahan
    $('#<%= Inflector::classify($singularVar) %>KecamatanId').change(function() {
        var propinsiId = $('#<%= Inflector::classify($singularVar) %>PropinsiId').val();
        var kabupatenId = $('#<%= Inflector::classify($singularVar) %>KabupatenId').val();
        ...
        if (propinsiId && kabupatenId && kecamatanId) {
            $('.address-loading').show();
            $.ajax({
                type: 'POST',
                url: '<?= $this->Url->build(["controller" => "Daerahs", "action" => "optionKelurahan"]) ?>/id_propinsi:' + propinsiId + '/id_kabupaten:' + kabupatenId + '/id_kecamatan:' + kecamatanId,
                cache: false,
                success: function(html) {
                    kelurahanSelect.html(html);
                    $('.address-loading').hide();
                },
                error: function() {
                    $('.address-loading').hide();
                    alert('Error loading Kelurahan data');
            });
    });
});
</script>
