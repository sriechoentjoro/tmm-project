 ...
 * @var \App\Model\Entity\ApprenticeFamily[]|\Cake\Collection\CollectionInterface $apprenticeFamilies
 */
?>
    ...
    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 15px;">
        <div style="display: flex; align-items: center; gap: 10px;">
            ...
            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                <?= $this->Html->link(
                    '<i class="fas fa-list"></i> ' . __('List Apprentice Families'),
                    ['action' => 'index'],
                    ['class' => 'dropdown-item', 'escape' => false]
                ) ?>
                <?= $this->Html->link(
                    '<i class="fas fa-plus"></i> ' . __('New Apprentice Family'),
                    ['action' => 'add'],
                    ['class' => 'dropdown-item', 'escape' => false]
                ) ?>
                ...
                <div class="dropdown-divider"></div>
                <?= $this->Html->link(
                    '<i class="fas fa-list"></i> ' . __('List Master Family Connections'),
                    ['controller' => 'MasterFamilyConnections', 'action' => 'index'],
                    ['class' => 'dropdown-item', 'escape' => false]
                ) ?>
                <?= $this->Html->link(
                    '<i class="fas fa-plus"></i> ' . __('New Master Family Connection'),
                    ['controller' => 'MasterFamilyConnections', 'action' => 'add'],
                    ['class' => 'dropdown-item', 'escape' => false]
                ) ?>
                <div class="dropdown-divider"></div>
                <?= $this->Html->link(
                    '<i class="fas fa-list"></i> ' . __('List Master Occupations'),
                    ['controller' => 'MasterOccupations', 'action' => 'index'],
                    ['class' => 'dropdown-item', 'escape' => false]
                ) ?>
                <?= $this->Html->link(
                    '<i class="fas fa-plus"></i> ' . __('New Master Occupation'),
                    ['controller' => 'MasterOccupations', 'action' => 'add'],
                    ['class' => 'dropdown-item', 'escape' => false]
                ) ?>
            </div>
            
            <h2 style="margin: 0; display: inline-block;"><?= __('Apprentice Families') ?></h2>
        </div>
...
<div class="table-scroll-wrapper" style="overflow-x: auto; cursor: grab; -webkit-overflow-scrolling: touch; user-select: none;">
    <div class="apprenticeFamilies index content">
        <table class="table" style="border-collapse: collapse; width: 100%; min-width: 800px;">
            ...
            <thead style="background: linear-gradient(135deg, rgba(102, 126, 234, 0.15) 0%, rgba(118, 75, 162, 0.15) 100%); position: sticky; top: 0; z-index: 10;">
                <tr>
                    ...
                    <th style="padding: 12px; border-bottom: 2px solid #667eea; white-space: nowrap;" scope="col"><?= $this->Paginator->sort('master_family_connection_id') ?></th>
                    <th style="padding: 12px; border-bottom: 2px solid #667eea; white-space: nowrap;" scope="col"><?= $this->Paginator->sort('name') ?></th>
                    <th style="padding: 12px; border-bottom: 2px solid #667eea; white-space: nowrap;" scope="col"><?= $this->Paginator->sort('age') ?></th>
                    <th style="padding: 12px; border-bottom: 2px solid #667eea; white-space: nowrap;" scope="col"><?= $this->Paginator->sort('master_occupation_id') ?></th>
                </tr>
            ...
            <tr class="filter-row" style="background-color: #f8f9fa;">
                ...
                <td style="padding: 5px;">
                    <select class="filter-input form-control form-control-sm" data-column="master_family_connection_id" data-type="select" style="font-size: 0.85rem; padding: 4px;">
                        <option value="">All Master_family_connections</option>
                        <?php foreach ($master_family_connections as $id => $name): ?>
                            <option value="<?= $id ?>"><?= h($name) ?></option>
                        <?php endforeach; ?>
                    </select>
                </td>
                ...
                <td style="padding: 5px;">
                    <select class="filter-operator form-control form-control-sm" data-column="age" style="font-size: 0.75rem; padding: 2px 4px; margin-bottom: 2px;">
                        <option value="=">=</option>
                        <option value="!=">!=</option>
                        <option value="<">&lt;</option>
                        <option value=">">&gt;</option>
                    ...
                    <input type="number" class="filter-input form-control form-control-sm" placeholder="Filter..." data-column="age" style="font-size: 0.85rem; padding: 4px;">
                    <input type="number" class="filter-input-range form-control form-control-sm" placeholder="To..." data-column="age" style="display:none; margin-top: 2px; font-size: 0.85rem;">
                </td>
                <td style="padding: 5px;">
                    <select class="filter-input form-control form-control-sm" data-column="master_occupation_id" data-type="select" style="font-size: 0.85rem; padding: 4px;">
                        <option value="">All Master_occupations</option>
                        <?php foreach ($master_occupations as $id => $name): ?>
                            <option value="<?= $id ?>"><?= h($name) ?></option>
                        <?php endforeach; ?>
                    </select>
                </td>
            ...
            <tbody>
                <?php foreach ($apprenticeFamilies as $apprenticeFamily): ?>
                <tr>
                    ...
                    <td class="actions" style="white-space: nowrap; padding: 8px;">
                        <div class="action-buttons-hover">
                            <?= $this->Html->link(
                                ...
                                ['action' => 'view', $apprenticeFamily->id],
                                ['class' => 'btn-action-icon btn-view-icon', 'escape' => false, 'title' => 'View']
                            ) ?>
                            <?= $this->Html->link(
                                '<i class="fas fa-edit"></i>',
                                ['action' => 'edit', $apprenticeFamily->id],
                                ['class' => 'btn-action-icon btn-edit-icon', 'escape' => false, 'title' => 'Edit']
                            ) ?>
                            <?= $this->Form->postLink(
                                '<i class="fas fa-trash"></i>',
                                ['action' => 'delete', $apprenticeFamily->id],
                                ['confirm' => __('Are you sure you want to delete # {0}?', $apprenticeFamily->id), 'class' => 'btn-action-icon btn-delete-icon', 'escape' => false, 'title' => 'Delete']
                            ) ?>
                        </div>
                    </td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $this->Number->format($apprenticeFamily->id) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $apprenticeFamily->has('apprentice') ? $this->Html->link($apprenticeFamily->apprentice->name, ['controller' => 'Apprentices', 'action' => 'view', $apprenticeFamily->apprentice->id]) : '' ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $apprenticeFamily->has('master_family_connection') ? $this->Html->link($apprenticeFamily->master_family_connection->title, ['controller' => 'MasterFamilyConnections', 'action' => 'view', $apprenticeFamily->master_family_connection->id]) : '' ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($apprenticeFamily->name) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $this->Number->format($apprenticeFamily->age) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $apprenticeFamily->has('master_occupation') ? $this->Html->link($apprenticeFamily->master_occupation->title, ['controller' => 'MasterOccupations', 'action' => 'view', $apprenticeFamily->master_occupation->id]) : '' ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
