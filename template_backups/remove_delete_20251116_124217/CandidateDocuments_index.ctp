 ...
 * @var \App\Model\Entity\CandidateDocument[]|\Cake\Collection\CollectionInterface $candidateDocuments
 */
?>
    ...
    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 15px;">
        <div style="display: flex; align-items: center; gap: 10px;">
            ...
            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                <?= $this->Html->link(
                    '<i class="fas fa-list"></i> ' . __('List Candidate Documents'),
                    ['action' => 'index'],
                    ['class' => 'dropdown-item', 'escape' => false]
                ) ?>
                <?= $this->Html->link(
                    '<i class="fas fa-plus"></i> ' . __('New Candidate Document'),
                    ['action' => 'add'],
                    ['class' => 'dropdown-item', 'escape' => false]
                ) ?>
            ...
            <h2 style="margin: 0; display: inline-block;"><?= __('Candidate Documents') ?></h2>
        </div>
...
<div class="table-scroll-wrapper" style="overflow-x: auto; cursor: grab; -webkit-overflow-scrolling: touch; user-select: none;">
    <div class="candidateDocuments index content">
        <table class="table" style="border-collapse: collapse; width: 100%; min-width: 800px;">
            ...
            <thead style="background: linear-gradient(135deg, rgba(102, 126, 234, 0.15) 0%, rgba(118, 75, 162, 0.15) 100%); position: sticky; top: 0; z-index: 10;">
                <tr>
                    ...
                    <th style="padding: 12px; border-bottom: 2px solid #667eea; white-space: nowrap;" scope="col"><?= $this->Paginator->sort('file_name') ?></th>
            ...
            <tr class="filter-row" style="background-color: #f8f9fa;">
                ...
                <td style="padding: 5px;">
                    <select class="filter-operator form-control form-control-sm" data-column="file_name" style="font-size: 0.75rem; padding: 2px 4px; margin-bottom: 2px;">
                        <option value="like">LIKE</option>
                        <option value="not_like">NOT LIKE</option>
                        <option value="=">=</option>
                        <option value="!=">!=</option>
                    ...
                    <input type="text" class="filter-input form-control form-control-sm" placeholder="Filter..." data-column="file_name" style="font-size: 0.85rem; padding: 4px;">
                </td>
                <td style="padding: 5px;">
            ...
            <tbody>
                <?php foreach ($candidateDocuments as $candidateDocument): ?>
                <tr>
                    ...
                    <td class="actions" style="white-space: nowrap; padding: 8px;">
                        <div class="action-buttons-hover">
                            <?= $this->Html->link(
                                ...
                                ['action' => 'view', $candidateDocument->id],
                                ['class' => 'btn-action-icon btn-view-icon', 'escape' => false, 'title' => 'View']
                            ) ?>
                            <?= $this->Html->link(
                                '<i class="fas fa-edit"></i>',
                                ['action' => 'edit', $candidateDocument->id],
                                ['class' => 'btn-action-icon btn-edit-icon', 'escape' => false, 'title' => 'Edit']
                            ) ?>
                            <?= $this->Form->postLink(
                                '<i class="fas fa-trash"></i>',
                                ['action' => 'delete', $candidateDocument->id],
                                ['confirm' => __('Are you sure you want to delete # {0}?', $candidateDocument->id), 'class' => 'btn-action-icon btn-delete-icon', 'escape' => false, 'title' => 'Delete']
                            ) ?>
                        </div>
                    </td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $this->Number->format($candidateDocument->id) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $candidateDocument->has('candidate') ? $this->Html->link($candidateDocument->candidate->name, ['controller' => 'Candidates', 'action' => 'view', $candidateDocument->candidate->id]) : '' ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidateDocument->title) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidateDocument->file_name) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidateDocument->comments) ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
