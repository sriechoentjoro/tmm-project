    ...
    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 15px;">
        <div style="display: flex; align-items: center; gap: 10px;">
            ...
            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                ...
                <div class="dropdown-divider"></div>
                <?= $this->Html->link(
                    '<i class="fas fa-list"></i> ' . __('List Master Departments'),
                    ['controller' => 'MasterDepartments', 'action' => 'index'],
                    ['class' => 'dropdown-item', 'escape' => false]
                ) ?>
                <?= $this->Html->link(
                    '<i class="fas fa-plus"></i> ' . __('New Master Department'),
                    ['controller' => 'MasterDepartments', 'action' => 'add'],
                    ['class' => 'dropdown-item', 'escape' => false]
                ) ?>
            </div>
            
            <h2 style="margin: 0; display: inline-block;"><?= __('Users') ?></h2>
        </div>
        ...
        <table class="table" style="border-collapse: collapse; width: 100%; min-width: 800px;">
            ...
            <thead style="background: linear-gradient(135deg, rgba(102, 126, 234, 0.15) 0%, rgba(118, 75, 162, 0.15) 100%); position: sticky; top: 0; z-index: 10;">
                <tr>
                    ...
                    <th style="padding: 12px; border-bottom: 2px solid #667eea; white-space: nowrap;" scope="col"><?= $this->Paginator->sort('code') ?></th>
                    ...
                    <th style="padding: 12px; border-bottom: 2px solid #667eea; white-space: nowrap;" scope="col"><?= $this->Paginator->sort('master_position_id') ?></th>
                    <th style="padding: 12px; border-bottom: 2px solid #667eea; white-space: nowrap;" scope="col"><?= $this->Paginator->sort('email') ?></th>
                    <th style="padding: 12px; border-bottom: 2px solid #667eea; white-space: nowrap;" scope="col"><?= $this->Paginator->sort('master_department_id') ?></th>
            ...
            <tr class="filter-row" style="background-color: #f8f9fa;">
                ...
                <td style="padding: 5px;">
                    <select class="filter-operator form-control form-control-sm" data-column="code" style="font-size: 0.75rem; padding: 2px 4px; margin-bottom: 2px;">
                    ...
                    <input type="text" class="filter-input form-control form-control-sm" placeholder="Filter..." data-column="code" style="font-size: 0.85rem; padding: 4px;">
                </td>
                <td style="padding: 5px;">
                ...
                <td style="padding: 5px;">
                    <select class="filter-input form-control form-control-sm" data-column="master_position_id" data-type="select" style="font-size: 0.85rem; padding: 4px;">
                        <option value="">All Master_positions</option>
                        <?php foreach ($master_positions as $id => $name): ?>
                            <option value="<?= $id ?>"><?= h($name) ?></option>
                        <?php endforeach; ?>
                    </select>
                </td>
                ...
                <td style="padding: 5px;">
                    <select class="filter-input form-control form-control-sm" data-column="master_department_id" data-type="select" style="font-size: 0.85rem; padding: 4px;">
                        <option value="">All Master_departments</option>
                        <?php foreach ($master_departments as $id => $name): ?>
                            <option value="<?= $id ?>"><?= h($name) ?></option>
                        <?php endforeach; ?>
                    </select>
                </td>
                    ...
                    <td class="actions" style="white-space: nowrap; padding: 8px;">
                        <div class="action-buttons-hover">
                            ...
                            <?= $this->Form->postLink(
                                ...
                                ['confirm' => __('Are you sure you want to delete # {0}?', $user->id), 'class' => 'btn-action-icon btn-delete-icon', 'escape' => false, 'title' => 'Delete']
                            ) ?>
                        </div>
                    </td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $this->Number->format($user->id) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($user->code) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($user->name) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $user->has('master_gender') ? $this->Html->link($user->master_gender->title, ['controller' => 'MasterGenders', 'action' => 'view', $user->master_gender->id]) : '' ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $this->Number->format($user->master_position_id) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($user->email) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $user->has('master_department') ? $this->Html->link($user->master_department->title, ['controller' => 'MasterDepartments', 'action' => 'view', $user->master_department->id]) : '' ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($user->updated) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($user->created) ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
