 ...
 * @var \App\Model\Entity\CandidateFamily[]|\Cake\Collection\CollectionInterface $candidateFamilies
 */
?>
    ...
    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 15px;">
        <div style="display: flex; align-items: center; gap: 10px;">
            ...
            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                <?= $this->Html->link(
                    '<i class="fas fa-list"></i> ' . __('List Candidate Families'),
                    ['action' => 'index'],
                    ['class' => 'dropdown-item', 'escape' => false]
                ) ?>
                <?= $this->Html->link(
                    '<i class="fas fa-plus"></i> ' . __('New Candidate Family'),
                    ['action' => 'add'],
                    ['class' => 'dropdown-item', 'escape' => false]
                ) ?>
            ...
            <h2 style="margin: 0; display: inline-block;"><?= __('Candidate Families') ?></h2>
        </div>
...
<div class="table-scroll-wrapper" style="overflow-x: auto; cursor: grab; -webkit-overflow-scrolling: touch; user-select: none;">
    <div class="candidateFamilies index content">
        <table class="table" style="border-collapse: collapse; width: 100%; min-width: 800px;">
            ...
            <tbody>
                <?php foreach ($candidateFamilies as $candidateFamily): ?>
                <tr>
                    ...
                    <td class="actions" style="white-space: nowrap; padding: 8px;">
                        <div class="action-buttons-hover">
                            <?= $this->Html->link(
                                ...
                                ['action' => 'view', $candidateFamily->id],
                                ['class' => 'btn-action-icon btn-view-icon', 'escape' => false, 'title' => 'View']
                            ) ?>
                            <?= $this->Html->link(
                                '<i class="fas fa-edit"></i>',
                                ['action' => 'edit', $candidateFamily->id],
                                ['class' => 'btn-action-icon btn-edit-icon', 'escape' => false, 'title' => 'Edit']
                            ) ?>
                            <?= $this->Form->postLink(
                                '<i class="fas fa-trash"></i>',
                                ['action' => 'delete', $candidateFamily->id],
                                ['confirm' => __('Are you sure you want to delete # {0}?', $candidateFamily->id), 'class' => 'btn-action-icon btn-delete-icon', 'escape' => false, 'title' => 'Delete']
                            ) ?>
                        </div>
                    </td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $this->Number->format($candidateFamily->id) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $candidateFamily->has('candidate') ? $this->Html->link($candidateFamily->candidate->name, ['controller' => 'Candidates', 'action' => 'view', $candidateFamily->candidate->id]) : '' ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $candidateFamily->has('master_family_connection') ? $this->Html->link($candidateFamily->master_family_connection->title, ['controller' => 'MasterFamilyConnections', 'action' => 'view', $candidateFamily->master_family_connection->id]) : '' ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidateFamily->name) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $this->Number->format($candidateFamily->age) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $candidateFamily->has('master_occupation') ? $this->Html->link($candidateFamily->master_occupation->title, ['controller' => 'MasterOccupations', 'action' => 'view', $candidateFamily->master_occupation->id]) : '' ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
