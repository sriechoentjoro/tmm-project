    ...
    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 15px;">
        <div style="display: flex; align-items: center; gap: 10px;">
            ...
            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                ...
                <div class="dropdown-divider"></div>
                <?= $this->Html->link(
                    '<i class="fas fa-list"></i> ' . __('List Master Candidate Interview Results'),
                    ['controller' => 'MasterCandidateInterviewResults', 'action' => 'index'],
                    ['class' => 'dropdown-item', 'escape' => false]
                ) ?>
                <?= $this->Html->link(
                    '<i class="fas fa-plus"></i> ' . __('New Master Candidate Interview Result'),
            ...
            <h2 style="margin: 0; display: inline-block;"><?= __('Candidates') ?></h2>
        </div>
        ...
        <table class="table" style="border-collapse: collapse; width: 100%; min-width: 800px;">
            ...
            <thead style="background: linear-gradient(135deg, rgba(102, 126, 234, 0.15) 0%, rgba(118, 75, 162, 0.15) 100%); position: sticky; top: 0; z-index: 10;">
                <tr>
                    ...
                    <th style="padding: 12px; border-bottom: 2px solid #667eea; white-space: nowrap;" scope="col"><?= __('Photo') ?></th>
                    <th style="padding: 12px; border-bottom: 2px solid #667eea; white-space: nowrap;" scope="col"><?= $this->Paginator->sort('id') ?></th>
                    <th style="padding: 12px; border-bottom: 2px solid #667eea; white-space: nowrap;" scope="col"><?= $this->Paginator->sort('apprenticeship_order_id') ?></th>
                    <th style="padding: 12px; border-bottom: 2px solid #667eea; white-space: nowrap;" scope="col"><?= $this->Paginator->sort('candidate_code') ?></th>
                    ...
                    <th style="padding: 12px; border-bottom: 2px solid #667eea; white-space: nowrap;" scope="col"><?= $this->Paginator->sort('master_blood_type_id') ?></th>
                    ...
                    <th style="padding: 12px; border-bottom: 2px solid #667eea; white-space: nowrap;" scope="col"><?= $this->Paginator->sort('master_candidate_interview_result_id') ?></th>
            ...
            <tr class="filter-row" style="background-color: #f8f9fa;">
                ...
                <td style="padding: 5px;">
                    <select class="filter-input form-control form-control-sm" data-column="apprenticeship_order_id" data-type="select" style="font-size: 0.85rem; padding: 4px;">
                ...
                <td style="padding: 5px;">
                    <select class="filter-operator form-control form-control-sm" data-column="candidate_code" style="font-size: 0.75rem; padding: 2px 4px; margin-bottom: 2px;">
                    ...
                    <input type="text" class="filter-input form-control form-control-sm" placeholder="Filter..." data-column="candidate_code" style="font-size: 0.85rem; padding: 4px;">
                </td>
                <td style="padding: 5px;">
                ...
                <td style="padding: 5px;">
                    <select class="filter-input form-control form-control-sm" data-column="master_blood_type_id" data-type="select" style="font-size: 0.85rem; padding: 4px;">
                        <option value="">All Master_blood_types</option>
                        <?php foreach ($master_blood_types as $id => $name): ?>
                            <option value="<?= $id ?>"><?= h($name) ?></option>
                        <?php endforeach; ?>
                    </select>
                </td>
                ...
                <td style="padding: 5px;">
                    <select class="filter-input form-control form-control-sm" data-column="master_candidate_interview_result_id" data-type="select" style="font-size: 0.85rem; padding: 4px;">
                        <option value="">All Master_candidate_interview_results</option>
                        <?php foreach ($master_candidate_interview_results as $id => $name): ?>
                            <option value="<?= $id ?>"><?= h($name) ?></option>
                        <?php endforeach; ?>
                    </select>
                </td>
                ...
                <tr>
                    ...
                    <td class="actions" style="white-space: nowrap; padding: 8px;">
                        <div class="action-buttons-hover">
                            ...
                            <?= $this->Form->postLink(
                                ...
                                ['confirm' => __('Are you sure you want to delete # {0}?', $candidate->id), 'class' => 'btn-action-icon btn-delete-icon', 'escape' => false, 'title' => 'Delete']
                            ) ?>
                        </div>
                    </td>
                    <!-- Photo Thumbnail -->
                    <td style="padding: 8px; text-align: center;">
                        <?php if (!empty($candidate->image_photo)): ?>
                            <?php 
                            // Check both with and without subdirectory path
                            $imagePath = 'uploads/' . $candidate->image_photo;
                            if (!file_exists(WWW_ROOT . 'img' . DS . 'uploads' . DS . $candidate->image_photo)) {
                                // Try with candidates subdirectory
                                if (file_exists(WWW_ROOT . 'img' . DS . 'uploads' . DS . 'candidates' . DS . $candidate->image_photo)) {
                                    $imagePath = 'uploads/candidates/' . $candidate->image_photo;
                                }
                            }
                            ?>
                            <?= $this->Html->image($imagePath, [
                                'alt' => h($candidate->name),
                                'style' => 'width: 50px; height: 50px; object-fit: cover; border-radius: 4px; cursor: pointer;',
                                'onclick' => 'window.open(this.src, \'_blank\')',
                                'onerror' => 'this.style.display=\'none\'; this.nextElementSibling.style.display=\'inline\';'
                            ]) ?>
                            <i class="fas fa-user-circle" style="font-size: 40px; color: #ccc; display: none;"></i>
                        <?php else: ?>
                            <i class="fas fa-user-circle" style="font-size: 40px; color: #ccc;"></i>
                        <?php endif; ?>
                    </td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $this->Number->format($candidate->id) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $this->Number->format($candidate->apprenticeship_order_id) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidate->candidate_code) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $this->Number->format($candidate->vocational_training_institution_id) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $this->Number->format($candidate->acceptance_organization_id) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidate->identity_number) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidate->name) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidate->name_katakana) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $candidate->has('master_gender') ? $this->Html->link($candidate->master_gender->title, ['controller' => 'MasterGenders', 'action' => 'view', $candidate->master_gender->id]) : '' ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $candidate->has('master_religion') ? $this->Html->link($candidate->master_religion->title, ['controller' => 'MasterReligions', 'action' => 'view', $candidate->master_religion->id]) : '' ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $candidate->has('master_marriage_status') ? $this->Html->link($candidate->master_marriage_status->title, ['controller' => 'MasterMarriageStatuses', 'action' => 'view', $candidate->master_marriage_status->id]) : '' ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidate->birth_place) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidate->birth_place_katakana) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidate->birth_date) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidate->telephone_mobile) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidate->telephone_emergency) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidate->email) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $candidate->has('master_propinsi') ? $this->Html->link($candidate->master_propinsi->title, ['controller' => 'MasterPropinsis', 'action' => 'view', $candidate->master_propinsi->id]) : '' ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $candidate->has('master_kabupaten') ? $this->Html->link($candidate->master_kabupaten->title, ['controller' => 'MasterKabupatens', 'action' => 'view', $candidate->master_kabupaten->id]) : '' ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $candidate->has('master_kecamatan') ? $this->Html->link($candidate->master_kecamatan->title, ['controller' => 'MasterKecamatans', 'action' => 'view', $candidate->master_kecamatan->id]) : '' ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $candidate->has('master_kelurahan') ? $this->Html->link($candidate->master_kelurahan->title, ['controller' => 'MasterKelurahans', 'action' => 'view', $candidate->master_kelurahan->id]) : '' ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $this->Number->format($candidate->post_code) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidate->address) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidate->image_photo) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidate->strengths) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidate->weaknesses) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidate->hobby) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $this->Number->format($candidate->last_salary_amount) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidate->application_reasons) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidate->is_ever_went_to_japan) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidate->will_go_to_japan_after_finished) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidate->expected_work_upon_returning_to_japan) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidate->is_holding_passport) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $this->Number->format($candidate->saving_goal_amount) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $candidate->has('master_blood_type') ? $this->Html->link($candidate->master_blood_type->title, ['controller' => 'MasterBloodTypes', 'action' => 'view', $candidate->master_blood_type->id]) : '' ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $this->Number->format($candidate->body_weight) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $this->Number->format($candidate->body_height) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidate->is_wear_eye_glasses) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidate->explain_eye_condition) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidate->is_color_blind) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidate->explain_color_blind) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidate->is_right_handed) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidate->is_smoking) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidate->is_drinking_alcohol) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidate->is_tattooed) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidate->link_whatsapp) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidate->link_line) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidate->link_instagram) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidate->link_facebook) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidate->link_tiktok) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $candidate->has('master_candidate_interview_result') ? $this->Html->link($candidate->master_candidate_interview_result->title, ['controller' => 'MasterCandidateInterviewResults', 'action' => 'view', $candidate->master_candidate_interview_result->id]) : '' ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidate->is_training_pass) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidate->is_apprenticeship_pass) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $candidate->has('master_rejected_reason') ? $this->Html->link($candidate->master_rejected_reason->title, ['controller' => 'MasterRejectedReasons', 'action' => 'view', $candidate->master_rejected_reason->id]) : '' ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
