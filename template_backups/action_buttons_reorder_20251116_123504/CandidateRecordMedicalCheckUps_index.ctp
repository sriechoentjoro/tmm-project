 ...
 * @var \App\Model\Entity\CandidateRecordMedicalCheckUp[]|\Cake\Collection\CollectionInterface $candidateRecordMedicalCheckUps
 */
?>
    ...
    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 15px;">
        <div style="display: flex; align-items: center; gap: 10px;">
            ...
            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                <?= $this->Html->link(
                    '<i class="fas fa-list"></i> ' . __('List Candidate Record Medical Check Ups'),
                    ['action' => 'index'],
                    ['class' => 'dropdown-item', 'escape' => false]
                ) ?>
                <?= $this->Html->link(
                    '<i class="fas fa-plus"></i> ' . __('New Candidate Record Medical Check Up'),
                    ['action' => 'add'],
                    ['class' => 'dropdown-item', 'escape' => false]
                ) ?>
            </div>
            
            <h2 style="margin: 0; display: inline-block;"><?= __('Candidate Record Medical Check Ups') ?></h2>
        </div>
...
<div class="table-scroll-wrapper" style="overflow-x: auto; cursor: grab; -webkit-overflow-scrolling: touch; user-select: none;">
    <div class="candidateRecordMedicalCheckUps index content">
        <table class="table" style="border-collapse: collapse; width: 100%; min-width: 800px;">
            ...
            <thead style="background: linear-gradient(135deg, rgba(102, 126, 234, 0.15) 0%, rgba(118, 75, 162, 0.15) 100%); position: sticky; top: 0; z-index: 10;">
                <tr>
                    ...
                    <th style="padding: 12px; border-bottom: 2px solid #667eea; white-space: nowrap;" scope="col"><?= $this->Paginator->sort('medical_check_up_result_id') ?></th>
            ...
            <tr class="filter-row" style="background-color: #f8f9fa;">
                ...
                <td style="padding: 5px;">
                    <select class="filter-input form-control form-control-sm" data-column="medical_check_up_result_id" data-type="select" style="font-size: 0.85rem; padding: 4px;">
                        <option value="">All Medical_check_up_results</option>
                        <?php foreach ($medical_check_up_results as $id => $name): ?>
                            <option value="<?= $id ?>"><?= h($name) ?></option>
                        <?php endforeach; ?>
                    </select>
                </td>
            ...
            <tbody>
                <?php foreach ($candidateRecordMedicalCheckUps as $candidateRecordMedicalCheckUp): ?>
                <tr>
                    ...
                    <td class="actions" style="white-space: nowrap; padding: 8px;">
                        <div class="action-buttons-hover">
                            <?= $this->Html->link(
                                ...
                                ['action' => 'view', $candidateRecordMedicalCheckUp->id],
                                ['class' => 'btn-action-icon btn-view-icon', 'escape' => false, 'title' => 'View']
                            ) ?>
                            <?= $this->Html->link(
                                '<i class="fas fa-edit"></i>',
                                ['action' => 'edit', $candidateRecordMedicalCheckUp->id],
                                ['class' => 'btn-action-icon btn-edit-icon', 'escape' => false, 'title' => 'Edit']
                            ) ?>
                            <?= $this->Form->postLink(
                                '<i class="fas fa-trash"></i>',
                                ['action' => 'delete', $candidateRecordMedicalCheckUp->id],
                                ['confirm' => __('Are you sure you want to delete # {0}?', $candidateRecordMedicalCheckUp->id), 'class' => 'btn-action-icon btn-delete-icon', 'escape' => false, 'title' => 'Delete']
                            ) ?>
                        </div>
                    </td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $this->Number->format($candidateRecordMedicalCheckUp->id) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $this->Number->format($candidateRecordMedicalCheckUp->applicant_id) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidateRecordMedicalCheckUp->title) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidateRecordMedicalCheckUp->date_issued) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $this->Number->format($candidateRecordMedicalCheckUp->medical_check_up_result_id) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidateRecordMedicalCheckUp->clinic) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidateRecordMedicalCheckUp->mcu_files) ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
