 ...
 * @var \App\Model\Entity\CandidateDocumentsMasterList[]|\Cake\Collection\CollectionInterface $candidateDocumentsMasterList
 */
?>
    ...
    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 15px;">
        <div style="display: flex; align-items: center; gap: 10px;">
            ...
            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                <?= $this->Html->link(
                    '<i class="fas fa-list"></i> ' . __('List Candidate Documents Master List'),
                    ['action' => 'index'],
                    ['class' => 'dropdown-item', 'escape' => false]
                ) ?>
                <?= $this->Html->link(
                    '<i class="fas fa-plus"></i> ' . __('New Candidate Documents Master List'),
                    ['action' => 'add'],
                    ['class' => 'dropdown-item', 'escape' => false]
                ) ?>
            </div>
            
            <h2 style="margin: 0; display: inline-block;"><?= __('Candidate Documents Master List') ?></h2>
        </div>
...
<div class="table-scroll-wrapper" style="overflow-x: auto; cursor: grab; -webkit-overflow-scrolling: touch; user-select: none;">
    <div class="candidateDocumentsMasterList index content">
        <table class="table" style="border-collapse: collapse; width: 100%; min-width: 800px;">
            ...
            <thead style="background: linear-gradient(135deg, rgba(102, 126, 234, 0.15) 0%, rgba(118, 75, 162, 0.15) 100%); position: sticky; top: 0; z-index: 10;">
                <tr>
                    ...
                    <th style="padding: 12px; border-bottom: 2px solid #667eea; white-space: nowrap;" scope="col"><?= $this->Paginator->sort('category_id') ?></th>
            ...
            <tr class="filter-row" style="background-color: #f8f9fa;">
                ...
                <td style="padding: 5px;">
                    <select class="filter-input form-control form-control-sm" data-column="category_id" data-type="select" style="font-size: 0.85rem; padding: 4px;">
            ...
            <tbody>
                <?php foreach ($candidateDocumentsMasterList as $candidateDocumentsMasterList): ?>
                <tr>
                    ...
                    <td class="actions" style="white-space: nowrap; padding: 8px;">
                        <div class="action-buttons-hover">
                            <?= $this->Html->link(
                                ...
                                ['action' => 'view', $candidateDocumentsMasterList->id],
                                ['class' => 'btn-action-icon btn-view-icon', 'escape' => false, 'title' => 'View']
                            ) ?>
                            <?= $this->Html->link(
                                '<i class="fas fa-edit"></i>',
                                ['action' => 'edit', $candidateDocumentsMasterList->id],
                                ['class' => 'btn-action-icon btn-edit-icon', 'escape' => false, 'title' => 'Edit']
                            ) ?>
                            <?= $this->Form->postLink(
                                '<i class="fas fa-trash"></i>',
                                ['action' => 'delete', $candidateDocumentsMasterList->id],
                                ['confirm' => __('Are you sure you want to delete # {0}?', $candidateDocumentsMasterList->id), 'class' => 'btn-action-icon btn-delete-icon', 'escape' => false, 'title' => 'Delete']
                            ) ?>
                        </div>
                    </td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $this->Number->format($candidateDocumentsMasterList->id) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidateDocumentsMasterList->title) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $this->Number->format($candidateDocumentsMasterList->category_id) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($candidateDocumentsMasterList->is_required) ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
