 ...
 * @var \App\Model\Entity\ApprenticeExperience[]|\Cake\Collection\CollectionInterface $apprenticeExperiences
 */
?>
    ...
    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 15px;">
        <div style="display: flex; align-items: center; gap: 10px;">
            ...
            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                <?= $this->Html->link(
                    '<i class="fas fa-list"></i> ' . __('List Apprentice Experiences'),
                    ['action' => 'index'],
                    ['class' => 'dropdown-item', 'escape' => false]
                ) ?>
                <?= $this->Html->link(
                    '<i class="fas fa-plus"></i> ' . __('New Apprentice Experience'),
                    ['action' => 'add'],
                    ['class' => 'dropdown-item', 'escape' => false]
                ) ?>
                ...
                <?= $this->Html->link(
                    ...
                    ['controller' => 'MasterEmployeeStatuses', 'action' => 'index'],
                    ['class' => 'dropdown-item', 'escape' => false]
                ) ?>
                <?= $this->Html->link(
                    '<i class="fas fa-plus"></i> ' . __('New Master Employee Status'),
                    ['controller' => 'MasterEmployeeStatuses', 'action' => 'add'],
                    ['class' => 'dropdown-item', 'escape' => false]
                ) ?>
            </div>
            
            <h2 style="margin: 0; display: inline-block;"><?= __('Apprentice Experiences') ?></h2>
        </div>
...
<div class="table-scroll-wrapper" style="overflow-x: auto; cursor: grab; -webkit-overflow-scrolling: touch; user-select: none;">
    <div class="apprenticeExperiences index content">
        <table class="table" style="border-collapse: collapse; width: 100%; min-width: 800px;">
            ...
            <thead style="background: linear-gradient(135deg, rgba(102, 126, 234, 0.15) 0%, rgba(118, 75, 162, 0.15) 100%); position: sticky; top: 0; z-index: 10;">
                <tr>
                    ...
                    <th style="padding: 12px; border-bottom: 2px solid #667eea; white-space: nowrap;" scope="col"><?= $this->Paginator->sort('employment_start_year') ?></th>
                    <th style="padding: 12px; border-bottom: 2px solid #667eea; white-space: nowrap;" scope="col"><?= $this->Paginator->sort('employment_end_year') ?></th>
                    <th style="padding: 12px; border-bottom: 2px solid #667eea; white-space: nowrap;" scope="col"><?= $this->Paginator->sort('company_name') ?></th>
                    <th style="padding: 12px; border-bottom: 2px solid #667eea; white-space: nowrap;" scope="col"><?= $this->Paginator->sort('title') ?></th>
                    <th style="padding: 12px; border-bottom: 2px solid #667eea; white-space: nowrap;" scope="col"><?= $this->Paginator->sort('master_employee_status_id') ?></th>
            ...
            <tr class="filter-row" style="background-color: #f8f9fa;">
                ...
                <td style="padding: 5px;">
                    <select class="filter-operator form-control form-control-sm" data-column="employment_start_year" style="font-size: 0.75rem; padding: 2px 4px; margin-bottom: 2px;">
                    ...
                    <input type="text" class="filter-input form-control form-control-sm" placeholder="Filter..." data-column="employment_start_year" style="font-size: 0.85rem; padding: 4px;">
                </td>
                <td style="padding: 5px;">
                    <select class="filter-operator form-control form-control-sm" data-column="employment_end_year" style="font-size: 0.75rem; padding: 2px 4px; margin-bottom: 2px;">
                        <option value="like">LIKE</option>
                        <option value="not_like">NOT LIKE</option>
                        <option value="=">=</option>
                        <option value="!=">!=</option>
                    ...
                    <input type="text" class="filter-input form-control form-control-sm" placeholder="Filter..." data-column="employment_end_year" style="font-size: 0.85rem; padding: 4px;">
                </td>
                <td style="padding: 5px;">
                    <select class="filter-operator form-control form-control-sm" data-column="company_name" style="font-size: 0.75rem; padding: 2px 4px; margin-bottom: 2px;">
                        <option value="like">LIKE</option>
                        <option value="not_like">NOT LIKE</option>
                        <option value="=">=</option>
                        <option value="!=">!=</option>
                    ...
                    <input type="text" class="filter-input form-control form-control-sm" placeholder="Filter..." data-column="company_name" style="font-size: 0.85rem; padding: 4px;">
                </td>
                <td style="padding: 5px;">
                ...
                <td style="padding: 5px;">
                    <select class="filter-input form-control form-control-sm" data-column="master_employee_status_id" data-type="select" style="font-size: 0.85rem; padding: 4px;">
                        <option value="">All Master_employee_statuss</option>
                        <?php foreach ($master_employee_statuss as $id => $name): ?>
                            <option value="<?= $id ?>"><?= h($name) ?></option>
                        <?php endforeach; ?>
                    </select>
                </td>
            ...
            <tbody>
                <?php foreach ($apprenticeExperiences as $apprenticeExperience): ?>
                <tr>
                    ...
                    <td class="actions" style="white-space: nowrap; padding: 8px;">
                        <div class="action-buttons-hover">
                            <?= $this->Html->link(
                                ...
                                ['action' => 'edit', $apprenticeExperience->id],
                                ['class' => 'btn-action-icon btn-edit-icon', 'escape' => false, 'title' => 'Edit']
                            ) ?>
                            <?= $this->Html->link(
                                '<i class="fas fa-eye"></i>',
                                ['action' => 'view', $apprenticeExperience->id],
                                ['class' => 'btn-action-icon btn-view-icon', 'escape' => false, 'title' => 'View']
                            ) ?>
                            <?= $this->Form->postLink(
                                '<i class="fas fa-trash"></i>',
                                ['action' => 'delete', $apprenticeExperience->id],
                                ['confirm' => __('Are you sure you want to delete # {0}?', $apprenticeExperience->id), 'class' => 'btn-action-icon btn-delete-icon', 'escape' => false, 'title' => 'Delete']
                            ) ?>
                        </div>
                    </td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $this->Number->format($apprenticeExperience->id) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $apprenticeExperience->has('apprentice') ? $this->Html->link($apprenticeExperience->apprentice->name, ['controller' => 'Apprentices', 'action' => 'view', $apprenticeExperience->apprentice->id]) : '' ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($apprenticeExperience->employment_start_year) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($apprenticeExperience->employment_end_year) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($apprenticeExperience->company_name) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($apprenticeExperience->title) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $apprenticeExperience->has('master_employee_status') ? $this->Html->link($apprenticeExperience->master_employee_status->title, ['controller' => 'MasterEmployeeStatuses', 'action' => 'view', $apprenticeExperience->master_employee_status->id]) : '' ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $this->Number->format($apprenticeExperience->last_salary_amount) ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
