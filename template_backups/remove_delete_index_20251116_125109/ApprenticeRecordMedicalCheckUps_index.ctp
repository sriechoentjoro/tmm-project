 ...
 * @var \App\Model\Entity\ApprenticeRecordMedicalCheckUp[]|\Cake\Collection\CollectionInterface $apprenticeRecordMedicalCheckUps
 */
?>
    ...
    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 15px;">
        <div style="display: flex; align-items: center; gap: 10px;">
            ...
            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                <?= $this->Html->link(
                    '<i class="fas fa-list"></i> ' . __('List Apprentice Record Medical Check Ups'),
                    ['action' => 'index'],
                    ['class' => 'dropdown-item', 'escape' => false]
                ) ?>
                <?= $this->Html->link(
                    '<i class="fas fa-plus"></i> ' . __('New Apprentice Record Medical Check Up'),
                    ['action' => 'add'],
                    ['class' => 'dropdown-item', 'escape' => false]
                ) ?>
                ...
                <div class="dropdown-divider"></div>
                <?= $this->Html->link(
                    '<i class="fas fa-list"></i> ' . __('List Master Medical Check Up Results'),
                    ['controller' => 'MasterMedicalCheckUpResults', 'action' => 'index'],
                    ['class' => 'dropdown-item', 'escape' => false]
                ) ?>
                <?= $this->Html->link(
                    '<i class="fas fa-plus"></i> ' . __('New Master Medical Check Up Result'),
                    ['controller' => 'MasterMedicalCheckUpResults', 'action' => 'add'],
                    ['class' => 'dropdown-item', 'escape' => false]
                ) ?>
            </div>
            
            <h2 style="margin: 0; display: inline-block;"><?= __('Apprentice Record Medical Check Ups') ?></h2>
        </div>
...
<div class="table-scroll-wrapper" style="overflow-x: auto; cursor: grab; -webkit-overflow-scrolling: touch; user-select: none;">
    <div class="apprenticeRecordMedicalCheckUps index content">
        <table class="table" style="border-collapse: collapse; width: 100%; min-width: 800px;">
            ...
            <thead style="background: linear-gradient(135deg, rgba(102, 126, 234, 0.15) 0%, rgba(118, 75, 162, 0.15) 100%); position: sticky; top: 0; z-index: 10;">
                <tr>
                    ...
                    <th style="padding: 12px; border-bottom: 2px solid #667eea; white-space: nowrap;" scope="col"><?= $this->Paginator->sort('date_issued') ?></th>
                    <th style="padding: 12px; border-bottom: 2px solid #667eea; white-space: nowrap;" scope="col"><?= $this->Paginator->sort('master_medical_check_up_result_id') ?></th>
                    <th style="padding: 12px; border-bottom: 2px solid #667eea; white-space: nowrap;" scope="col"><?= $this->Paginator->sort('clinic') ?></th>
                    <th style="padding: 12px; border-bottom: 2px solid #667eea; white-space: nowrap;" scope="col"><?= $this->Paginator->sort('mcu_files') ?></th>
                </tr>
            ...
            <tr class="filter-row" style="background-color: #f8f9fa;">
                ...
                <td style="padding: 5px;">
                    <select class="filter-operator form-control form-control-sm" data-column="date_issued" style="font-size: 0.75rem; padding: 2px 4px; margin-bottom: 2px;">
                        <option value="=">=</option>
                        <option value="!=">!=</option>
                        <option value="<">&lt;</option>
                        <option value=">">&gt;</option>
                    ...
                    <input type="date" class="filter-input form-control form-control-sm" data-column="date_issued" style="font-size: 0.85rem; padding: 4px;">
                    <input type="date" class="filter-input-range form-control form-control-sm" data-column="date_issued" style="display:none; margin-top: 2px; font-size: 0.85rem;">
                </td>
                <td style="padding: 5px;">
                    <select class="filter-input form-control form-control-sm" data-column="master_medical_check_up_result_id" data-type="select" style="font-size: 0.85rem; padding: 4px;">
                        <option value="">All Master_medical_check_up_results</option>
                        <?php foreach ($master_medical_check_up_results as $id => $name): ?>
                            <option value="<?= $id ?>"><?= h($name) ?></option>
                        <?php endforeach; ?>
                    </select>
                </td>
                <td style="padding: 5px;">
                    <select class="filter-operator form-control form-control-sm" data-column="clinic" style="font-size: 0.75rem; padding: 2px 4px; margin-bottom: 2px;">
                        <option value="like">LIKE</option>
                        <option value="not_like">NOT LIKE</option>
                        <option value="=">=</option>
                        <option value="!=">!=</option>
                    ...
                    <input type="text" class="filter-input form-control form-control-sm" placeholder="Filter..." data-column="clinic" style="font-size: 0.85rem; padding: 4px;">
                </td>
                <td style="padding: 5px;">
                    <select class="filter-operator form-control form-control-sm" data-column="mcu_files" style="font-size: 0.75rem; padding: 2px 4px; margin-bottom: 2px;">
                        <option value="like">LIKE</option>
                        <option value="not_like">NOT LIKE</option>
                        <option value="=">=</option>
                        <option value="!=">!=</option>
                    ...
                    <input type="text" class="filter-input form-control form-control-sm" placeholder="Filter..." data-column="mcu_files" style="font-size: 0.85rem; padding: 4px;">
                </td>
            </tr>
            </thead>
            
            <tbody>
                <?php foreach ($apprenticeRecordMedicalCheckUps as $apprenticeRecordMedicalCheckUp): ?>
                <tr>
                    ...
                    <td class="actions" style="white-space: nowrap; padding: 8px;">
                        <div class="action-buttons-hover">
                            <?= $this->Html->link(
                                ...
                                ['action' => 'edit', $apprenticeRecordMedicalCheckUp->id],
                                ['class' => 'btn-action-icon btn-edit-icon', 'escape' => false, 'title' => 'Edit']
                            ) ?>
                            <?= $this->Html->link(
                                '<i class="fas fa-eye"></i>',
                                ['action' => 'view', $apprenticeRecordMedicalCheckUp->id],
                                ['class' => 'btn-action-icon btn-view-icon', 'escape' => false, 'title' => 'View']
                            ) ?>
                            <?= $this->Form->postLink(
                                '<i class="fas fa-trash"></i>',
                                ['action' => 'delete', $apprenticeRecordMedicalCheckUp->id],
                                ['confirm' => __('Are you sure you want to delete # {0}?', $apprenticeRecordMedicalCheckUp->id), 'class' => 'btn-action-icon btn-delete-icon', 'escape' => false, 'title' => 'Delete']
                            ) ?>
                        </div>
                    </td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $this->Number->format($apprenticeRecordMedicalCheckUp->id) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $apprenticeRecordMedicalCheckUp->has('apprentice') ? $this->Html->link($apprenticeRecordMedicalCheckUp->apprentice->name, ['controller' => 'Apprentices', 'action' => 'view', $apprenticeRecordMedicalCheckUp->apprentice->id]) : '' ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($apprenticeRecordMedicalCheckUp->title) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($apprenticeRecordMedicalCheckUp->date_issued) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $apprenticeRecordMedicalCheckUp->has('master_medical_check_up_result') ? $this->Html->link($apprenticeRecordMedicalCheckUp->master_medical_check_up_result->title, ['controller' => 'MasterMedicalCheckUpResults', 'action' => 'view', $apprenticeRecordMedicalCheckUp->master_medical_check_up_result->id]) : '' ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($apprenticeRecordMedicalCheckUp->clinic) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($apprenticeRecordMedicalCheckUp->mcu_files) ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
