 ...
 * @var \App\Model\Entity\Authorization[]|\Cake\Collection\CollectionInterface $authorizations
 */
?>
    ...
    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 15px;">
        <div style="display: flex; align-items: center; gap: 10px;">
            ...
            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                <?= $this->Html->link(
                    '<i class="fas fa-list"></i> ' . __('List Authorizations'),
                    ['action' => 'index'],
                    ['class' => 'dropdown-item', 'escape' => false]
                ) ?>
                <?= $this->Html->link(
                    '<i class="fas fa-plus"></i> ' . __('New Authorization'),
                    ['action' => 'add'],
                    ['class' => 'dropdown-item', 'escape' => false]
                ) ?>
                ...
                <div class="dropdown-divider"></div>
                <?= $this->Html->link(
                    '<i class="fas fa-list"></i> ' . __('List Master Authorization Roles'),
                    ['controller' => 'MasterAuthorizationRoles', 'action' => 'index'],
                    ['class' => 'dropdown-item', 'escape' => false]
                ) ?>
                <?= $this->Html->link(
                    '<i class="fas fa-plus"></i> ' . __('New Master Authorization Role'),
                    ['controller' => 'MasterAuthorizationRoles', 'action' => 'add'],
                    ['class' => 'dropdown-item', 'escape' => false]
                ) ?>
            </div>
            
            <h2 style="margin: 0; display: inline-block;"><?= __('Authorizations') ?></h2>
        </div>
...
<div class="table-scroll-wrapper" style="overflow-x: auto; cursor: grab; -webkit-overflow-scrolling: touch; user-select: none;">
    <div class="authorizations index content">
        <table class="table" style="border-collapse: collapse; width: 100%; min-width: 800px;">
            ...
            <thead style="background: linear-gradient(135deg, rgba(102, 126, 234, 0.15) 0%, rgba(118, 75, 162, 0.15) 100%); position: sticky; top: 0; z-index: 10;">
                <tr>
                    ...
                    <th style="padding: 12px; border-bottom: 2px solid #667eea; white-space: nowrap;" scope="col"><?= $this->Paginator->sort('user_id') ?></th>
                    <th style="padding: 12px; border-bottom: 2px solid #667eea; white-space: nowrap;" scope="col"><?= $this->Paginator->sort('master_authorization_role_id') ?></th>
                    <th style="padding: 12px; border-bottom: 2px solid #667eea; white-space: nowrap;" scope="col"><?= $this->Paginator->sort('user_name') ?></th>
                    <th style="padding: 12px; border-bottom: 2px solid #667eea; white-space: nowrap;" scope="col"><?= $this->Paginator->sort('password') ?></th>
                    <th style="padding: 12px; border-bottom: 2px solid #667eea; white-space: nowrap;" scope="col"><?= $this->Paginator->sort('last_login') ?></th>
            ...
            <tr class="filter-row" style="background-color: #f8f9fa;">
                ...
                <td style="padding: 5px;">
                    <select class="filter-input form-control form-control-sm" data-column="user_id" data-type="select" style="font-size: 0.85rem; padding: 4px;">
                ...
                <td style="padding: 5px;">
                    <select class="filter-input form-control form-control-sm" data-column="master_authorization_role_id" data-type="select" style="font-size: 0.85rem; padding: 4px;">
                        <option value="">All Master_authorization_roles</option>
                        <?php foreach ($master_authorization_roles as $id => $name): ?>
                            <option value="<?= $id ?>"><?= h($name) ?></option>
                        <?php endforeach; ?>
                    </select>
                </td>
                <td style="padding: 5px;">
                    <select class="filter-operator form-control form-control-sm" data-column="user_name" style="font-size: 0.75rem; padding: 2px 4px; margin-bottom: 2px;">
                    ...
                    <input type="text" class="filter-input form-control form-control-sm" placeholder="Filter..." data-column="user_name" style="font-size: 0.85rem; padding: 4px;">
                </td>
                <td style="padding: 5px;">
                    <select class="filter-operator form-control form-control-sm" data-column="password" style="font-size: 0.75rem; padding: 2px 4px; margin-bottom: 2px;">
                        <option value="like">LIKE</option>
                        <option value="not_like">NOT LIKE</option>
                        <option value="=">=</option>
                        <option value="!=">!=</option>
                    ...
                    <input type="text" class="filter-input form-control form-control-sm" placeholder="Filter..." data-column="password" style="font-size: 0.85rem; padding: 4px;">
                </td>
                <td style="padding: 5px;">
                    <select class="filter-operator form-control form-control-sm" data-column="last_login" style="font-size: 0.75rem; padding: 2px 4px; margin-bottom: 2px;">
                        <option value="=">=</option>
                        <option value="!=">!=</option>
                        <option value="<">&lt;</option>
                        <option value=">">&gt;</option>
                    ...
                    <input type="date" class="filter-input form-control form-control-sm" data-column="last_login" style="font-size: 0.85rem; padding: 4px;">
                    <input type="date" class="filter-input-range form-control form-control-sm" data-column="last_login" style="display:none; margin-top: 2px; font-size: 0.85rem;">
                </td>
                <td style="padding: 5px;">
                    ...
                    <td class="actions" style="white-space: nowrap; padding: 8px;">
                        <div class="action-buttons-hover">
                            <?= $this->Html->link(
                                ...
                                ['action' => 'edit', $authorization->id],
                                ['class' => 'btn-action-icon btn-edit-icon', 'escape' => false, 'title' => 'Edit']
                            ) ?>
                            <?= $this->Html->link(
                                '<i class="fas fa-eye"></i>',
                                ['action' => 'view', $authorization->id],
                                ['class' => 'btn-action-icon btn-view-icon', 'escape' => false, 'title' => 'View']
                            ) ?>
                            <?= $this->Form->postLink(
                                '<i class="fas fa-trash"></i>',
                                ['action' => 'delete', $authorization->id],
                                ['confirm' => __('Are you sure you want to delete # {0}?', $authorization->id), 'class' => 'btn-action-icon btn-delete-icon', 'escape' => false, 'title' => 'Delete']
                            ) ?>
                        </div>
                    </td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $this->Number->format($authorization->id) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $authorization->has('user') ? $this->Html->link($authorization->user->name, ['controller' => 'Users', 'action' => 'view', $authorization->user->id]) : '' ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= $authorization->has('master_authorization_role') ? $this->Html->link($authorization->master_authorization_role->title, ['controller' => 'MasterAuthorizationRoles', 'action' => 'view', $authorization->master_authorization_role->id]) : '' ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($authorization->user_name) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($authorization->password) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($authorization->last_login) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($authorization->updated) ?></td>
                    <td style="padding: 8px; white-space: nowrap;"><?= h($authorization->created) ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
