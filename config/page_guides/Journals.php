...
return [
    ...
    'cautions' => [
        ...
        __('If a source of entries cannot be read at all, the page says so at the top rather than showing a smaller number. An unreadable source used to look exactly like having nothing left to journalize.'),
        __('A source record whose mapping needs an account the Chart of Accounts does not have is held back the same way, and says which account is missing. It used to be passed over in silence: the record stayed on the pending list, counted in the pending total, and Generate All did nothing with it - so the page promised an entry the button never made.'),
